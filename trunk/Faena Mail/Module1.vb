...
Imports System.IO
Imports FaenaMailLibrary
Module Module1
    Public reg As New ClassRegistry
    Public gblnProductID As Boolean = False
    Public cntNewMailNotify As Integer = 0
    ...
    Public Enum wTypeEnum
        ...
        MB_OK = &H0
    End Enum
    Public Declare Function MessageBeep Lib "user32.dll" Alias "MessageBeep" ( _
    ByVal wType As wTypeEnum) As Int32
    ...
    Public Sub MarkMailStatus(ByVal filePath As String, ByVal status As String)
        Try
            ...
            While RD.Peek > 0
                ...
                If line = "" Then
                    If Not found Then
                        buf = buf & "Status: " & status & vbNewLine
                    End If
                    buf = buf & vbNewLine
                    Exit While
                Else
                    buf = buf & line & vbNewLine
                End If
            End While
            buf = buf & RD.ReadToEnd
            RD.Close()
            FS.Close()
            FS = New FileStream(filePath, FileMode.Open, FileAccess.Write)
            Dim WD As New StreamWriter(FS)
            WD.Write(buf)
            WD.Close()
            FS.Close()
...
Label1Exit:
    ...
    Public Function EmailPart(ByVal strIn As String) As String
        If strIn Like "*<*@*>*" Then
            Return strIn.Substring(strIn.IndexOf("<") + 1, strIn.IndexOf(">") - strIn.IndexOf("<") - 1)
